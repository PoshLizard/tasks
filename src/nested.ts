...
export function sumPublishedPoints(questions: Question[]): number {
    return questions.reduce((sum, q) => (q.published ? sum + q.points : sum), 0);
}
...
export function toCSV(questions: Question[]): string {
    return "id,name,options,points,published\n" + questions.map((q) => `${q.id},${q.name},${q.options.length},${q.points},${q.published}`).join("\n");
}
...
export function duplicateQuestionInArray(
    ...
    return questions.reduce((acc: Question[], q) => {
        acc.push(q);
        if (q.id === targetId) {
            const duplicate = {
                ...q,
